...
public class MQConfig {
     ...
     */
    @Bean
    public DirectExchange aiPictureExchange() {
        // 参数：交换机名、持久化、自动删除
        return new DirectExchange(RabbitMqAiConstant.AI_PICTURE_EXCHANGE, true, false);
    }
     ...
     */
    @Bean
    public Queue aiPictureQueue() {
        Map<String, Object> args = new HashMap<>();
        // 死信配置：消息成为死信后，转发到DLX交换机+DLX路由键
        args.put("x-dead-letter-exchange", RabbitMqAiConstant.DLX_EXCHANGE);
        args.put("x-dead-letter-routing-key", RabbitMqAiConstant.DLX_ROUTING);
        args.put("x-message-ttl", 180000); // 消息60秒后过期
        // 队列属性：持久化、非独占、非自动删除
        return new Queue(RabbitMqAiConstant.AI_PICTURE_QUEUE, true, false, false, args);
    }
     ...
     */
    @Bean
    public Binding aiPictureBinding(Queue aiPictureQueue, DirectExchange aiPictureExchange) {
        return BindingBuilder.bind(aiPictureQueue)
                .to(aiPictureExchange)
                .with(RabbitMqAiConstant.AI_PICTURE_ROUTING); // 业务路由键
    }
